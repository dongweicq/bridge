...
Bridge 是一个 Android 应用，作为 OpenClaw 的"某信操作手"，通过 AccessibilityService 实现某信自动化操作。
...
| 工具 | 说明 | 前置工具 |
|------|------|----------|
| 搜索按钮 | 某信首页右上角放大镜 | 打开某信 |
| 输入法剪贴板 | 键盘上的剪贴板位置 | 打开某信 → 搜索按钮 |
| 联系人 | 搜索结果中的联系人 | 打开某信 → 搜索按钮 → 输入法剪贴板 |
| 消息输入框 | 聊天界面底部输入框 | ... → 联系人 |
...
查看日志：
```bash
adb logcat -s ToolManager:D Bridge:D MoxinAction:D
```
